...
Pipeline:
    ff/.../camera.obj  (Blender export, meters, optical axis +Z)
...
Usage:
    .venv/bin/python scripts/isaac/usd/bake_camera_mesh.py                 # bake + verify
    .venv/bin/python scripts/isaac/usd/bake_camera_mesh.py --dry-run       # measure only
    .venv/bin/python scripts/isaac/usd/bake_camera_mesh.py --verify-only   # re-check on disk
"""
...
DEFAULT_SOURCE = PROJECT_ROOT / "ff" / "Attach_20260706102846" / "camera.obj"
...
def write_obj(path: Path, verts32: np.ndarray, faces: np.ndarray, src: Path, n_src_faces: int) -> None:
    ...
    with open(tmp, "w") as f:
        ...
        f.write(f"# source: {src.relative_to(PROJECT_ROOT)} (untracked)  sha256 {sha256(src)}\n")
        f.write(f"# generated by scripts/isaac/usd/{Path(__file__).name}\n")
...
def main() -> None:
    ...
    ap.add_argument("--input", type=Path, default=DEFAULT_SOURCE, help="source OBJ (meters, +Z optical axis)")
    ...
    if not args.verify_only:
        ...
        mesh = load_and_rotate(args.input)
        ...
        if args.dry_run:
            ...
            return
        write_obj(OUT_OBJ, verts32, dec.faces, args.input, n_src_faces)
