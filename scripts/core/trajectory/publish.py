...
사용법:
    uv run scripts/core/trajectory/publish.py --csv <csv> --target isaac
    uv run scripts/core/trajectory/publish.py --csv <csv> --target controller
"""
...
try:
    ...
    # Keep --help and CSV parsing utilities available outside a sourced ROS2 shell.
    # A real publish exits with a clear message below before a placeholder class is used.
...
def main():
    ...
    if ROS_IMPORT_ERROR is not None:
        parser.error(
            ...
            f"before publishing ({ROS_IMPORT_ERROR})."
        )
    ...
    if args.joint_target is not None:
        ...
        if not Path(csv_path).exists():
            ...
            print("  Generate a trajectory first: glns/solve.py -> glns/verify.py")
            return
