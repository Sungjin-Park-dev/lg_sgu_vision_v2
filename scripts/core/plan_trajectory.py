...
# graph 가 redundant 하고 PRM 이 느림. graph off + via-roll 로 coverage 동일·5~9배 단축 검증
# (curved 99/100 27s, sample 69/74 56s, collisions=0). graph 없으면 성공은 attempt 0 에 끝나
# 실패만 max_attempts 만큼 헛도므로 max_attempts 도 낮게 둔다. (필요 시 graph 재활성: ATTEMPT=1)
TRANSIT_MAX_ATTEMPTS = 2            # plan_cspace 재시도 횟수 — 실패 빨리 포기(graph off 라 성공은 0에 끝)
...
def plan_reconfig_transits(
    ...
    print("    Warming up MotionPlanner (trajopt-only, no graph)...")
