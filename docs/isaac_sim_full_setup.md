...
[컨테이너 시작]  docker start / docker exec 로 진입
      ↓
[Isaac Sim 실행]  scripts/apps/isaac_pipeline.py
```
...
docker exec -it "$CTR" bash -c \
  'source /workspace/.venv/bin/activate && cd /workspace && \
   OMNI_KIT_ACCEPT_EULA=YES python scripts/apps/isaac_pipeline.py --object sample --mode sim'
```
...
| 스크립트 | 용도 |
|---|---|
| `scripts/apps/isaac_pipeline.py --object sample --mode sim` | Isaac 단독 물체 선택·궤적 생성·미리보기 |
| `scripts/apps/isaac_pipeline.py --object sample --mode real` | `/joint_states` 미러링 + 실로봇 HOME/궤적 전송 |
| `scripts/apps/isaac_pipeline.py --pipeline-mode moveit` | MoveIt 연동 |

앞부분(`source ... && cd /workspace && OMNI_KIT_ACCEPT_EULA=YES python`)은 공통.

### 프로세스 수동 종료
```bash
docker exec "$CTR" pkill -f "scripts/apps/isaac_pipeline.py"
```
