...
Isaac Sim Launcher 또는 설치 환경의 기본 실행 명령으로 빈 stage를 연다. 이 작업은
로봇 USD를 처음 만들 때만 필요하며, 일상적인 실행은 아래 `isaac_pipeline.py`를 사용한다.
...
생성한 USD와 워크셀 로딩, ROS2 graph 구성은 `isaac_pipeline.py`가 내부의
`core/isaac/scene.py`를 통해 처리한다. 사용자는 core CLI를 직접 실행하지 않는다.

---

## isaac_pipeline.py — 물체 선택·이동·궤적 생성/테스트

워크셀을 띄우고, Omni.UI 패널(Mode / Generate / Preview / Execute)로
**물체를 고르고, 뷰포트 기즈모로 옮긴 뒤, 그 위치에서 궤적을 생성·미리보기**한다.

```bash
uv run scripts/apps/isaac_pipeline.py --object sample            # 기본 sim 모드
uv run scripts/apps/isaac_pipeline.py --object sample --mode real # 실로봇 모드로 부팅
```
...
3. **궤적 생성** — `Generate Trajectory`. 물체의 현재 월드 pose를 읽어 robot frame으로 변환
   (`z -= 0.805`) 후 `trajectory/cli.py`에 `--object-position`/`--object-quat`로 전달.
   stage에 물체가 없으면 생성을 막고 안내한다(stale pose 방지).
4. **테스트** — Preview 패널 `Load & Preview` → `Play`. ghost UR20가 옮긴 물체를 향해
   검사 궤적을 재생한다.

### 새 물체 추가 (source.usd 선행 변환)

드롭다운은 `source.obj`가 있는 물체를 모두 보여주지만, 로드에는 `source.usd`가 필요하다.
새 물체를 추가하거나 source.obj를 변경했다면 변환한다(`omni.kit.asset_converter`):

```bash
# raw OBJ가 mm 단위라면 먼저 data/{object}/mesh/source.obj로 정규화
uv run scripts/setup/prepare_object_mesh.py normalize \
    --object curved_structure --input raw_export.obj

uv run scripts/setup/build_object_usd.py --object curved_structure
```
...
STP→OBJ 변환 시 메시에 박힌 이상한 방향은 normalize만으로는 고쳐지지 않는다.
Isaac에서 똑바로 안 서면:

1. UI에서 물체 로드 → 뷰포트에서 `E`(회전) 기즈모로 똑바로 세운다.
2. `Log Pose` 버튼 → 로그의 `--world-target-quat W X Y Z` 줄을 복사.
3. 베이크: `uv run scripts/setup/prepare_object_mesh.py reorient --object {obj} --world-target-quat W X Y Z`
   (config가 identity라 target을 그대로 베이크). 단순 90° 뒤집기는 `--euler -90 0 0`도 가능.
4. `uv run scripts/setup/build_object_usd.py --object {obj} --force` → source.usd 재생성.
...
# 셸 B: Isaac Pipeline (venv, 시스템 ROS2 X)
uv run --no-sync scripts/apps/isaac_pipeline.py --object sample --mode real
```

앱의 Home/Execute 패널에서 시작 자세 이동과 궤적 전송을 수행한다. mock hardware가 state를
반영하면 Isaac Sim 로봇이 이를 미러링한다.
...
저장 후 `scripts/apps/isaac_pipeline.py`를 재실행.
