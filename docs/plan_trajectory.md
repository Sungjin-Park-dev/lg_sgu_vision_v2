...
- `trajectory/publish.py`는 CSV의 `time`을 그대로 ROS trajectory time으로 사용한다.
...
joint 속도 제한, corner slowdown)은 `scripts/core/trajectory/settings.py`에서 관리한다.
...
```bash
uv run scripts/core/trajectory/cli.py \
