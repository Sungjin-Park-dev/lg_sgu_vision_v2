...
각 클러스터별로 원본 grid `row_index`를 사용해 `reorder_zigzag()` 수행. PCA 재투영으로 행을 재추정하지 않음 (곡면 클러스터에서의 오분류 방지 — [archive/zigzag-row-assignment-issue.md](archive/zigzag-row-assignment-issue.md) 참조).
...
`scripts/visualize_viewpoints.py`의 `visualize_clusters_html()`로 생성 (내부 전용, 직접 실행 안 함).
