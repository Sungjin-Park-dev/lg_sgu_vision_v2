...
**배치(sampling)** 와 **클러스터 내부 순서(ordering)** 는 각각 두 가지 모드를 가진다.

## 파이프라인

```
OBJ 로드 → (material RGB 필터)
  → [배치] grid: PCA 그리드 → 표면 스냅  |  surface: 표면 직접 균일 샘플링(Poisson-disk)
  → 법선/카메라 위치 → bottom-facing 필터 → 클러스터링
  → [순서] zigzag: 전역 PCA 행 정렬  |  graph: 탄젠트 Delaunay/TSP
...
| `surface` | 메시 표면 위에서 직접 균일 샘플링(`trimesh.sample_surface_even`, target count = `area/(2·spacing²)`). 곡면·측벽도 **표면적 기준**으로 고르게 덮음. | 곡면·입체물 (cylinder/curved/box) |
...
| `graph` | 클러스터 **평균 법선**의 탄젠트 평면에 투영 → `scipy.spatial.Delaunay`/kNN 인접 그래프 → nearest-neighbor + 2-opt open path. 전역 PCA 평면에 의존하지 않아 곡면 클러스터에 적합. tangent-정렬 baseline보다 길어지지 않도록 가드. |

> 보통 `surface`+`graph`, `grid`+`zigzag`를 쌍으로 쓴다 (viser 스튜디오는 자동 페어링).
...
| `--surface-spacing` | FOV 작은 축 mm | [surface] Poisson 표면 간격 |
| `--ordering-mode` | `zigzag` | `zigzag` \| `graph` (위 "순서 모드") |
...
**`graph`**: `order_cluster_graph()` — 클러스터 평균 법선으로 탄젠트 2D 프레임을 만들고(PCA 아님), `scipy.spatial.Delaunay`(degenerate면 kNN/tangent-정렬 폴백)로 인접 그래프를 구성한 뒤 nearest-neighbor + open-path 2-opt로 방문 순서를 푼다. 전역 PCA 평면에 의존하지 않아 곡면/입체 클러스터에 강하다. 결과 경로가 tangent-정렬 baseline보다 길면 폴백(절대 악화 없음). 반환은 `{sorted_indices, endpoint_a/b, normal_a/b}`로 zigzag와 동일 → GTSP 단계 그대로 동작. `n≤2`/공선/중복점/비유한값 모두 안전 처리(중복점은 dedup하지 않음 — path_order 무결성).
